    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-sm text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th @click="$wire.sortField('desc')" scope="col" class="px-4 py-3 cursor-pointer">
                    Deskripsi <x-sort :$sortDirection :$sortBy :field="'desc'" />
                </th>
        ...
        <tbody>
            <tr class="bg-gray-50 text-gray-700 border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td scope="col" class="px-4 py-2">
                    <x-input type="search" wire:model.live="form.desc" class="w-full text-sm" />
                </td>
                <td scope="col" class="px-4 py-2"></td>
                <td scope="col" class="px-4 py-2"></td>
            </tr>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-4 py-3 w-44">
                        <x-button-sm class="mr-1" type="button"
                            @click="$dispatch('produk-table-edit', {id: '{{ $item->id }}'})">Edit</x-button-sm>
                        <x-button-danger-sm
                            @click="$dispatch('produk-table-delete', {id: '{{ $item->id }}', name: '{{ $item->name }}'})">Delete</x-button-danger-sm>
                    </td>
                    <td class="px-4 py-3 text-center w-28">{{ $item->id }}</td>
                    <td class="px-4 py-3">{{ $item->name }}</td>
                    <td class="px-4 py-3">{{ $item->desc }}</td>
                    <td class="px-4 py-3">{{ $item->slug }}</td>
                    <td class="px-4 py-3">{{ $item->img }}</td>
                </tr>
            @empty
                Belum Ada Data
            @endforelse
